...
while True:
    ...
    if disparity is None:
        ...
        Distance = (7*10**6)*np.power(np.float16(disparity),-2.42)
        # Distance = (26.134)*np.power(np.float16(disparity),-0.392)
        # for x in range(disparity.shape[0]):
        #     for y in range(disparity.shape[1]):
        #         Distance[x][y] = round(317.29*disparity[x][y]**(-0.833),3)
        ...
        plt.ion()
        fig, ax = plt.subplots()
        im_display = ax.imshow(Distance, cmap='jet_r')
